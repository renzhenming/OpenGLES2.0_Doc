...
WarmTextureImageRender::WarmTextureImageRender(JNIEnv *env, jobject asset) {
    loader = new NativeAssetsLoader(env, asset);
}

WarmTextureImageRender::~WarmTextureImageRender() {

}

void WarmTextureImageRender::CreateSurface() {
    glClearColor(0.5f, 0.5f, 0.5f, 1.0f);
    int file_size = 0;
    unsigned char *vertex_shader_code = loader->LoadFile("warm_texture_image_vertex.vs", file_size);
    unsigned char *fragment_shader_code = loader->LoadFile("warm_texture_image_fragment.fs", file_size);
...
void WarmTextureImageRender::SurfaceChanged(int width, int height) {
...
void WarmTextureImageRender::DrawFrame() {
